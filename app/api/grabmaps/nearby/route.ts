// Node proxy for GrabMaps Places nearby search.
...
export async function GET(req: Request): Promise<Response> {
  ...
  const radiusKm = reqUrl.searchParams.get('radiusKm') || '2';
  const limit = reqUrl.searchParams.get('limit') || '120';
  ...
  try {
    ...
    const upstream = buildGrabUrl('/api/v1/maps/place/v2/nearby', {
      location: `${lat},${lng}`,
      radius: radiusKm,
      limit,
      rankBy: 'distance',
